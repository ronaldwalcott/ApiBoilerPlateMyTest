...
{
    public class PersonManagerCopy : DbFactoryBase, IPersonManager
    {
        private readonly ILogger<PersonManager> _logger;
//        public PersonManagerCopy(IConfiguration config, ILogger<PersonManager> logger) : base(config)
        public PersonManagerCopy(IConfiguration config, TestDBContext context, ILogger<PersonManager> logger) : base(config, context)
        {
            _logger = logger;
        }
